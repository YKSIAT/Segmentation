...
def get_mask_array(mask_slice_data, regions_type="whole", all_labels=False):
    ...
    try:
        ...
        # print("test test", np.unique(seg_labels[0], seg_labels[1], seg_labels[2]))
        ...
        # core_mask = (core_mask > 0).astype(int)
        # active_mask = (active_mask > 0).astype(int)
...
def get_near_the_three_layers(root_path, save_path):
    ...
    :param root_path:  root_path/case_file/*.nii.gz
    :param save_path: save_path/case_file/*.nii.gz
    :return:
    """
    ...
    for index, sub_dir in enumerate(sub_dirs):
        ...
        for idx in range(shape[2]):
            ...
            if mask_slice_data.sum() != 0:
                ...
                t2_slice_data = []
                if idx == 0:
                    flair_slice_data.append(standardization(standardization(flair_data[:, :, idx])))
                    flair_slice_data.append(standardization(flair_data[:, :, idx]))
                    flair_slice_data.append(standardization(flair_data[:, :, idx+1]))
                    flair_slice_data = np.array(flair_slice_data)

                    t1_slice_data.append(standardization(t1_data[:, :, idx]))
                    t1_slice_data.append(standardization(t1_data[:, :, idx]))
                    t1_slice_data.append(standardization(t1_data[:, :, idx+1]))
                    t1_slice_data = np.array(t1_slice_data)

                    t1ce_slice_data.append(standardization(t1ce_data[:, :, idx]))
                    t1ce_slice_data.append(standardization(t1ce_data[:, :, idx]))
                    t1ce_slice_data.append(standardization(t1ce_data[:, :, idx+1]))
                    t1ce_slice_data = np.array(t1ce_slice_data)

                    t2_slice_data.append(standardization(t2_data[:, :, idx]))
                    t2_slice_data.append(standardization(t2_data[:, :, idx]))
                    t2_slice_data.append(standardization(t2_data[:, :, idx+1]))
...
def generator_image_mask_three(data_path, batch_size=8):
    ...
    while True:
        ...
        for idx, value in enumerate(zip(all_data_slices, all_mask_slices)):
            ...
            image_batch_data.append(np.load(value[0]).astype(int))
            mask_batch_data.append([(np.load(value[1]) > 0).astype(int)])
            if idx == 0:
                continue
...
def generator_image_mask_three_test(data_path, batch_size=8):
    ...
    print(case_list)
    for idx, value in enumerate(case_list):
        npy_data_path = glob(os.path.join(data_path, value, "whole_wt", "*.npy"))
        print(idx, value, len(npy_data_path))
        pass
...
if __name__ == "__main__":
    ...
    p = [r"H:\N42HM\N4BiasedField\Train", r"H:\N42HM\N4BiasedField\Val"]
    s = [r"H:\N42HM\DataNpy_N4\Train_Norm", r"H:\N42HM\DataNpy_N4\Val_Norm"]
